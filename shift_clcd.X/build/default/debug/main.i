...
static void clcd_write(char, char);
...
void main(void) {
    init_config();
    clcd_putstr("Select DKP SW", (0x80 + 0));
    clcd_putstr("S0-Left S1-Right", (0xC0 + 0));
    ...
    while (1) {
        ...
        if(prekey == 0x3E)
        {
            clcd_putstr("Left Scroll", (0x80 + 0));
            temp = str[0];
            for(int i = 0; i < 15; i++)
            {
                str[i] = str[i+1];
            ...
            clcd_putstr(str, (0xC0 + 0));
        }

        else if(prekey == 0x3D)
        {
            clcd_putstr("Right Scroll", (0x80 + 0));
            temp = str[15];
            for(int i = 15; i >0; i--)
            {
                str[i] = str[i-1];
